...
object ClientOSRpt {
  def main(args: Array[String]): Unit = {
    ...
    sQLContext.sql(
      """
        |select
        |case when client = 1 then 'android' when client = 2 then 'ios' when client = 3 then 'wp' else '其他' end client,
      ...
      .jdbc(jdbcConfig.getUrl,"rpt_client_os",jdbcConfig.getConnectionProperties)
//      .show()
  }
}
